...
public class MyTree {
	...
	private int count(Node p) {
		...
		else return 1+count(p.leftChild)+count(p.rightChild); // 프로그램 완성 전에 프로그램을 실행해 볼 수 있도록 임시로 추가한 문장이므로 나중에 삭제하면 됨
	...
	public int getRight() {
		...
		return n.data; // 프로그램 완성 전에 프로그램을 실행해 볼 수 있도록 임시로 추가한 문장이므로 나중에 삭제하면 됨
	}
}